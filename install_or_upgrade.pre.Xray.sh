...
wget -q https://api.github.com/repos/XTLS/Xray-core/releases
tag=`grep "tag_name" releases | cut -d'"' -f4 | head -n 1`
...
then
	...
	if [ $a == 1 ]
	then 
		wget -q `grep browser_download_url releases| grep linux-64 | cut -d'"' -f4 | head -n 1`
	elif [ $a == 2 ]
	then
		wget -q `grep browser_download_url releases| grep linux-arm64 | cut -d'"' -f4 | head -n 1`
	else
