...
SERVICE_NAME="xcore"
XRAY_INIT="/etc/init.d/${SERVICE_NAME}"
...
# ===== 函数：Wrt 上生成 /etc/init.d/xcore（procd）和默认 config.json =====
ensure_xray_service() {
    # 1) /etc/init.d/xcore (procd)
    if [ ! -x "$XRAY_INIT" ]; then
        echo "Creating procd service at $XRAY_INIT ..."
        cat > "$XRAY_INIT" <<EOF
...
start_service() {
    ...
    # If your config uses geosite/geoip, keep them in the same folder:
    procd_set_param env XRAY_LOCATION_ASSET="\$XRAY_DIR"
    procd_close_instance
}
EOF
        chmod +x "$XRAY_INIT"
    else
        echo "Service $XRAY_INIT already exists, skip creating."
    fi
...
# 2) 确保 procd service + config.json 存在
ensure_xray_service
...
# 4) 用 /etc/init.d/xcore 重启
echo
echo "Restarting Xray via: /etc/init.d/${SERVICE_NAME} restart"
"/etc/init.d/${SERVICE_NAME}" restart || echo "WARNING: restart failed."
sleep 1

echo "Logs (OpenWrt): logread -e ${SERVICE_NAME}"
echo "Done."